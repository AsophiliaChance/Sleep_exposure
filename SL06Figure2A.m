...
% ?   subject-level waveform      mean    SE  
%   STD  -> Standard
%   DEV  -> Deviant
%   Diff1 -> Differential
% Grand average  ?       ?   ? ?   P2 / P450 ? ?  
...
for idev = 1:nDev
    ...
    for idata = 1:3
        ...
        %% ---------- component windows ----------
        rectangle('Position', ...
            [win.p21-0.2, componentBoxBottom, ...
             win.p22-win.p21, componentBoxHeight], ...
            'EdgeColor', mycolor(1,:), ...
            'LineStyle', '--', ...
            'LineWidth', 1);

        rectangle('Position', ...
            [win.p4501-0.2, componentBoxBottom, ...
             win.p4502-win.p4501, componentBoxHeight], ...
            'EdgeColor', mycolor(2,:), ...
            'LineStyle', '--', ...
            'LineWidth', 1);
        ...
        axis([plotTimeWin(1) plotTimeWin(2) plotYLim]);
    ...
    annotation('textbox', [0.3, 0.955, 0.4, 0.035], ...
        'String', change{idev}, ...
        'EdgeColor','none', ...
...
amplitude_diff.mmn = nan(nDay, maxSub, nDev);   % ? ? ? P2 amplitude
amplitude_diff.p3  = nan(nDay, maxSub, nDev);   % ? ? ? P450 amplitude

latency_diff.mmn   = nan(nDay, maxSub, nDev);   % ? ? ? P2 50% area latency
latency_diff.p3    = nan(nDay, maxSub, nDev);   % ? ? ? P450 50% area latency
...
for ivar = 1:numel(varList)
    ...
    for md = 1:nDayX
        ...
        if nValid > size(X,2)
            warning('%s Day %d: nValid > ?? subject      ?  ?? ', varList{ivar}, md);
            nValid = size(X,2);
