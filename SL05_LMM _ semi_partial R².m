...
for c = 1:numel(components)
    ...
    for idev = 1:nCond
        ...
        % R?_full
        yhat_fixed_full = predict(fullModel,'Predictor','Fixed');
        Var_fixed_full = var(yhat_fixed_full);
        Var_res_full   = fullModel.MSE;
        Var_rand_full  = sum(fullModel.VarianceComponents.Estimate);
        R2_full = Var_fixed_full / (Var_fixed_full + Var_rand_full + Var_res_full);

        % R?_null
        yhat_fixed_null = predict(nullModel,'Predictor','Fixed');
        Var_fixed_null = var(yhat_fixed_null);
        Var_res_null   = nullModel.MSE;
        Var_rand_null  = sum(nullModel.VarianceComponents.Estimate);
...
disp('? 完成：LMM + Semi-partial R? 已保存到 ERP_LMM_SemiPartialR2.xlsx');
