...
% 含交互作用时统一使用 effects coding。
% 这样 Days 主效应表示对 Small/Large 等权平均后的总体 Night 效应。
dummyCoding = 'effects';
...
for m = 1:2
    ...
    for ic = 1:numel(comps)
        ...
        lme_Exp_ML = fitlme(Tc_compare, formula_Exp, ...
            'FitMethod', 'ML', 'DummyVarCoding', dummyCoding);
        ...
        %% ---------- Night × DeviantType 交互作用 ----------
        interactionRow = find_interaction_effect_row(aovOut, 'Days', 'DeviantType');

        if isempty(interactionRow)
            warning('未在 ANOVA 表中找到 Days × DeviantType 交互作用: %s | %s', ...
                measureNames{m}, thisComp);
        else
            [Fval_int, df1_int, df2_int, pval_int] = ...
                extract_effect_values(aovOut, interactionRow);
            eta_int = calc_partial_eta2(Fval_int, df1_int, df2_int);

            allInteractionRows(rInteraction,:) = { ...
                measureNames{m}, ...
                thisComp, ...
                selectedModel, ...
                df1_int, ...
            ...
            rInteraction = rInteraction + 1;

            fprintf(['Night x DeviantType interaction: F(%s, %s) = %.3f, ' ...
                'p = %.4f, eta_p^2 = %.3f\n'], ...
                num2str(df1_int), num2str(df2_int), ...
                Fval_int, pval_int, eta_int);
        end

        %% ---------- planned marginal comparisons: Day2/3/4 vs Day1 ----------
        % 在含交互作用的模型中，对 Small/Large deviant 等权平均后比较各 Night 与 Night 1。
...
if isempty(allInteractionRows)
    T_interaction = table();
else
    T_interaction = cell2table(allInteractionRows, 'VariableNames', ...
        {'Measure','Component','SelectedModel','DF1','DF2','F','p','PartialEta2'});
end
...
if ~isempty(T_interaction)
    writetable(T_interaction, outFile, 'Sheet', 'Night_x_DeviantType');
end
...
% 在 effects coding 下，Days 主效应系数表示各 Night 相对于总体均值的偏差。
% 对 Small/Large 等权平均时，DeviantType 及 Days × DeviantType 项相互抵消，
% 因此 planned marginal comparison 只需要对 Days 主效应系数构造 contrast。
%
% 对 nDays = 4，effects coding 的 Day 编码为：
%   Day1 = [ 1  0  0]
%   Day2 = [ 0  1  0]
%   Day3 = [ 0  0  1]
%   Day4 = [-1 -1 -1]
%
% Day k vs Day1 的 contrast = code(Day k) - code(Day1)。
...
% effects coding 对 nDays 个水平生成 nDays-1 个 Days 系数。
...
for d = 1:(nDays-1)
    ...
    if isempty(idx)
        error(['未找到 effects coding 下的固定效应系数 %s。' ...
               '请检查 Days 的类别水平和 DummyVarCoding 设置。'], ...
               char(effectName));
    end
...
for i = 1:nCmp
    ...
    C = zeros(1, numel(beta));
    C(dayCoefIdx) = dayCode(targetDay,:) - referenceCode;

    Estimate(i) = C * beta;
    SE(i) = sqrt(max(C * covBeta * C', 0));

    try
        [pContrast, FContrast, ~, df2Contrast] = coefTest( ...
            lme, C, 0, 'DFMethod', 'Satterthwaite');

        DF(i) = df2Contrast;
        tval(i) = sign(Estimate(i)) * sqrt(FContrast);
        p_raw(i) = pContrast;

    catch ME
        warning('coefTest 对比失败，改用 t 分布近似：%s', ME.message);

        if SE(i) > 0
            tval(i) = Estimate(i) / SE(i);
            involvedDF = double(st.DF(C ~= 0));
            involvedDF = involvedDF(~isnan(involvedDF));

            if ~isempty(involvedDF)
                DF(i) = min(involvedDF);
                p_raw(i) = 2 * (1 - tcdf(abs(tval(i)), DF(i)));
            end
        end
    end
...
function idx = find_interaction_effect_row(aovOut, effectName1, effectName2)
...
for i = 1:numel(varNames)
    vn = lower(varNames{i});
    if contains(vn, 'term') || contains(vn, 'name') || contains(vn, 'source')
        candidateCols{end+1} = varNames{i}; %#ok<AGROW>
    end
end
...
for c = 1:numel(candidateCols)
    vals = string(aovOut.(candidateCols{c}));
    valsLower = lower(strtrim(vals));

    hit = find(contains(valsLower, lower(effectName1)) & ...
               contains(valsLower, lower(effectName2)) & ...
               contains(valsLower, ':'));
